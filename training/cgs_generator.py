    ...
    def __init__(
        ...
        **synthesis_kwargs,  # Arguments for SynthesisNetwork.
        ...
        self.mapping_network = MappingNetwork(
            ...
            c_dim=c_dim,
            w_dim=w_dim,
            num_ws=self.point_gen.num_ws + 1,
            **mapping_kwargs,
        )
    ...
    def synthesis(
        ...
        **synthesis_kwargs,
        ...
        (
            ...
            sample_color,
            sample_opacity,
        ) = self.point_gen(pos, encoded_pos, edge_index, ws)
        dec_out = {}
        ...
        for batch_idx in range(len(ws)):
            ...
            gaussian_params_i["_xyz_init"] = pos
            gaussian_params_i["_features_dc"] = (
                dec_out["color"][batch_idx].unsqueeze(1).contiguous()
            )  # self._features_dc # 3
            gaussian_params_i["_features_rest"] = (
                dec_out["color"][batch_idx].unsqueeze(1)[:, 0:0].contiguous()
            )  # self._features_rest # 3
            gaussian_params_i["_scaling"] = dec_out["scale"][
                batch_idx
            ]  # self._scaling # 3
            gaussian_params_i["_rotation"] = dec_out["rotation"][
                batch_idx
            ]  # self._rotation # 4
            gaussian_params_i["_opacity"] = dec_out["opacity"][
                batch_idx
            ]  # self._opacity # 1
