...
class ModLayer(nn.Module):
    def __init__(self, channels, z_dim, activation="leaky_relu", demod=True, use_noise=False):
        super().__init__()
        self.affine = nn.Linear(z_dim, channels)
    ...
    def forward(self, x, w):
        ...
        if self.use_noise:
            x = x + self.noise_strength * torch.randn_like(x)
...
class ModLinear(nn.Module):
    def __init__(self, in_channels, out_channels, z_dim, activation="leaky_relu", demod=True, use_noise=False):
        super().__init__()
...
class GaussianDecoder(nn.Module):
    def __init__(self, feature_channels: dict, in_channels, hidden_channels, z_dim):
        ...
        for key, channels in feature_channels.items():
            layer = ModLinear(hidden_channels, channels, z_dim, activation="identity", demod=False)
...
class CloudGenerator(nn.Module):
    def __init__(self, channels=128, num_pts=1024, z_dim=128, blocks=2):
        ...
        self.feature_blocks = nn.ModuleList(
            [
                ModLinear(256, 256, z_dim, use_noise=True),
                ModLinear(256, 256, z_dim, use_noise=True),
                ModLinear(256, 256, z_dim, use_noise=True),
                ModLinear(256, 256, z_dim, use_noise=True),
                ModLinear(256, 256, z_dim, use_noise=True),
