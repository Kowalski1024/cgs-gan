...
@click.option(
    "--cam_sample_mode",
    help="found in custom dist",
    type=str,
    default="smile_pose_rebalancing",
)
@click.option(
    "--outdir", help="Where to save the results", type=str, default="./training-results"
)
...
@click.option(
    "--network_snap", help="How often to save network pkl", type=int, default=500
)
...
@click.option(
    "--disc_c_noise",
    help="Strength of disc pose cond reg, in std.",
    type=float,
    default=0,
)
@click.option(
    "--g_num_fp16_res", help="Number of fp16 layers in generator", type=int, default=0
)
@click.option(
    "--d_num_fp16_res",
...
@click.option(
    "--gaussian_num_pts", help="the number of init gaussian.", type=int, default=512
)
...
@click.option(
    "--use_multivew_reg",
    help="compute grad for multiple views",
    type=bool,
    default=True,
)
@click.option(
    "--num_multiview",
    help="number of renderings per training step",
    type=int,
    default=4,
)
@click.option(
    "--anisotropy_penalty",
    help="penalty for anisotropic gaussians",
    type=float,
    default=0.01,
)
@click.option(
    "--position_reg",
    help="penalty for position (distance from origin)",
    type=float,
    default=1.0,
)
# Optional job description
@click.option(
    "--desc", help="String to include in result dir name", type=str, default="cgs_gan"
)
...
@click.option(
    "--resume_blur", help="Enable to blur even on resume", type=bool, default=False
)
def main(**kwargs):
    ...
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(
        class_name="training.dataset.CarsDataset",
        data=opts.data,
        cam_sample_mode=opts.cam_sample_mode,
    )
    ...
    c.loss_kwargs.loss_custom_options = {
        ...
        "anisotropy_penalty": opts.anisotropy_penalty,
    }
    c.loss_kwargs.blur_init_sigma = 10  # Blur the images seen by the discriminator.
    c.loss_kwargs.blur_fade_kimg = (
        c.batch_size * opts.blur_fade_kimg / 32
